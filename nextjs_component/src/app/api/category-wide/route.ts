...
export async function GET() {
  try {
    ...
    // Calculate totals (like Python's df['Column'].sum())
    const baselineTotalSales = mergedData.reduce((sum, record) => sum + record.BaselineSales, 0);
    const newTotalSales = mergedData.reduce((sum, record) => sum + record.NewSales, 0);
    
    const baselineTotalProfit = mergedData.reduce((sum, record) => sum + record.BaselineProfit, 0);
    const newTotalProfit = mergedData.reduce((sum, record) => sum + record.NewProfit, 0);
    
    const baselineTotalEmissions = mergedData.reduce((sum, record) => sum + record.BaselineEmissions, 0);
    const newTotalEmissions = mergedData.reduce((sum, record) => sum + record.NewEmissions, 0);
    
    const baselineTotalOffsetCost = mergedData.reduce((sum, record) => sum + record.BaselineOffsetCost, 0);
    const newTotalOffsetCost = mergedData.reduce((sum, record) => sum + record.NewOffsetCost, 0);
    ...
    console.log(getDescriptiveStats(mergedData.map(r => r.EstimatedUnitVolume)));
    console.log(getDescriptiveStats(mergedData.map(r => r.PredictedPrice)));
    console.log(getDescriptiveStats(mergedData.map(r => r.EstimatedSales)));
    
    console.log('\nMargin (as numeric):');
    console.log(getDescriptiveStats(mergedData.map(r => r.Margin)));
    
    console.log('\nTotal Emission per unit:');
    console.log(getDescriptiveStats(mergedData.map(r => r.Total_Emission_per_unit)));

    // Sample of product-level data
    console.log('\n=== Sample of Product-Level Data ===');
    mergedData.slice(0, 5).forEach(record => {
      console.log({
    ...
    console.error('Error in category-wide analysis:', error);
    return NextResponse.json(
      { error: 'Failed to process category-wide analysis' },
      { status: 500 }
    );
  }
} 